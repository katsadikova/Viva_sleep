#----------------------------------------------------------------------------#
#--- MSM_sleep_PROMIS_boys.R
#--- Date:    6/22/2023 (original)
#--- Description: Look at the effect of sleeping at least 8 hours per weeknight 
#---              over the course of adolescence (14-16Y) on PROMIS scores at 17
#----------------------------------------------------------------------------#
...
summary(complete(d_imp)$BMI_7y)
sqrt(var(complete(d_imp)$BMI_7y))
...
ages <- long %>% 
  ...
  mutate(
    ...
    time_rt_avg_14y = age-14.6,
    time_rt_avg_15y = age-15.6,
    sq_time_rt_avg_aphv = time_rt_avg_aphv*time_rt_avg_aphv,
...
#--------------------------------------------------------------------------------------------------------------#
#-- Among boys, look at a regime of consistently sleeping >8hr/night on PROMIS dep at 17y

#-- Run through imputations & save estimates for sleep8
...
for(i in seq(1:20)){
  ...
  b_da_long <- d_imp_long %>% filter(visit>12) 
  b_da_long <- data.frame(complete(b_da_long,i))
  
  exposure_weights_unstab <- ipwtm(
    exposure = sleep8,
    family = "binomial",
    link = "logit",
    denominator = ~  puberty_12y+BMI_7y+ses_index_new_03+age+as.factor(sex),
    id = aid,
    timevar = visit,
    type = "first",
    data = b_da_long
  ...
  b <- b_da_long %>%
    mutate(
      exp_weight_unstab =exposure_weights_unstab$ipw.weights)
  b <- data.frame(b)
  summary(b$exp_weight_stab)
  summary(b$exp_weight_unstab)
  
  
  mod1 <- svyglm(C5_depr_tscore ~ sleep8,
                 family = "gaussian",
                 design = svydesign(ids = ~aid, weights = ~exp_weight_unstab, data = b)
  )
  est <- round(summary(mod1)$coefficients[2,1],2)
  SE <- round(summary(mod1)$coefficients[2,2],2)
...
ests_mean
write.csv(ests_d, "/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/VDI/Viva/results_imp_chronological/msm_sleep_PROMIS_ests.csv")
write.csv(ests_mean, "/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/VDI/Viva/results_imp_chronological/msm_sleep_PROMIS_MEAN_est.csv")
